...
// --------------------------------------------------------------------------
// Original Code: RHCGI.cpp (RobinHood CGI Handler)
// All Modifications have been marked with "(this has been modified for PHP)"
// --------------------------------------------------------------------------
...
status_t handle_request( RequestPB *pb )
{
	...
	// PATH_TRANSLATED (this has been modified for PHP)
	if( absPath.Path() )
		env.PutEnv( "PATH_TRANSLATED", absPath.Path() );
	...
	// ****
	// (this has been modified for PHP)
		
	char *args[3];
	args[2] = "/boot/home/config/bin/php";
	args[0] = strdup(absPath.Path());
	args[1] = NULL;
	
	pb->Logprintf( "%ld PHP: %s\n", pb->sn, args[0], args[1] );
	...
	if( pid == (pid_t)0 ) // If we are the child process...
	{
		...
		execve( args[2], args, env.GetEnvironment() );
